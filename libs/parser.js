...
async function getCity(url, cityCode) {
    let body;
    try {
        const { stdout } = await exec(`curl -m 20 -A "Mozilla/5.0 (iPhone; CPU iPhone OS 11_0 like Mac OS X) AppleWebKit/604.1.38 (KHTML, like Gecko) Version/11.0 Mobile/15A372 Safari/604.1" https://${url}?cityId=City${cityCode}`, {maxBuffer: 1024 * 5000});
        body = stdout;
    } catch (err) {
